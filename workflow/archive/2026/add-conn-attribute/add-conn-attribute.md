# 工作项: add-conn-attribute

描述: statement 和 query 支持 `conn=<name>` 属性，指定命名连接，实现多连接并发测试（如事务锁竞争）
目标分支: main
源分支: add-conn-attribute
基线提交: a6c8719bc48099cf772a6bd1807876dd4577259c
文档影响: examples/demo2.slt 为并发测试示例

> **本文件须保存为 `workflow/archive/2026/add-conn-attribute/add-conn-attribute.md`**，文件名与目录同名。
> 流程定义见 `workflow/WORKFLOW.md`；看板见 `workflow/STATUS.md`。
> 本工作项的全部产物平铺在 `workflow/archive/2026/add-conn-attribute/`，无子目录、无版本后缀。
