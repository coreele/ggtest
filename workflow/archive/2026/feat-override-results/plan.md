...
- 依据 Spec: [`workflow/archive/2026/feat-override-results/spec.md`](./spec.md)（**唯一行为合同**）
...
预期证据：`mvn -q test` 全绿；`mvn -q clean package` 成功；两份 README 选项表含 `--override`（grep 可验证）。开发者验证结果记入 `workflow/archive/2026/feat-override-results/dev-notes.md`。
...
- Reviewer 须按 `workflow/agents/standards/quality.md` §3 检查：
...
- 取得 Approve 后，由 Manager 将状态推进至 QA 调度；QA 依据 Spec P0/P1 + 本 Plan 最低验证层独立验收，结论写入 `workflow/archive/2026/feat-override-results/qa-report.md`。

## 交接顺序

1. **实施（Developer）**：按 T1‖T2‖T5 → T3 → T4 → T6 → T7 → T8 实现；执行 `mvn -q test` 与 `mvn -q clean package`；将验证结果与偏差记入 `workflow/archive/2026/feat-override-results/dev-notes.md`。
