...
### 9. Git 合规
- 实现当前位于 `main` 且全部未提交（`git status` 显示工作区改动）。按 `workflow/agents/standards/git.md` §1.1，实现须位于切片源分支（推荐 `feat-statement-error-msg`），禁止在 `main` 直接实施。此项属 Manager/合并阶段处理范畴，不影响代码审阅结论，见「备注」。
...
- 工作流产物（spec/plan/manager 记录/STATUS）结构与目录命名符合 workflow/WORKFLOW.md 规范。
