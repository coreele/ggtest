...
提交须保持原子性：每次提交对应单一逻辑变更，便于审阅与回滚。代码与文档属不同逻辑变更，须分提（见 §2.4）。授权关闭时「STATUS/`done` + 未入库 review/qa 报告」视为同一逻辑关闭变更，允许同一次提交。
...
当一次变更同时含**实现/测试代码**与**工作流文档**（`dev-notes.md`、工作项记录、STATUS 等）时，**分两次提交，先代码后文档**：

1. **代码提交**（先）：`<type>(<scope>): ...` —— 仅含实现、测试及与之紧耦合的资源（fixtures、build 配置等）；
2. **文档提交**（后）：`docs(workflow): ...` —— 含 `dev-notes.md`、工作项记录与 STATUS 的状态推进等工作流文档。

理由：代码提交保持纯净，便于审阅、cherry-pick 与回滚；工作流文档作为代码变更的元数据记录单独成文。

**例外 / 补充：**

- `plan.md` 在 Planner 完成时即提交（先于代码），不并入代码提交。
- 关闭提交（`STATUS/done` + 未入库 `review.md` / `qa-report.md`）为**纯文档**，按 §1.4 与 README §Merge 一次提交，不拆分。
- 纯文档变更（无代码）直接用单次 `docs(...)` 提交。
