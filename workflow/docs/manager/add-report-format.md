# 工作项记录: add-report-format

工作项标识: add-report-format
...
## 切片门禁（未拆分，sub-feature-id = add-report-format）

| sub-feature-id | 路径等级 | 源分支 | Spec | Spec 门禁 | Spec 用户确认 | Design 门禁 | Review 门禁 |
|---|---|---|---|---|---|---|---|
| add-report-format | standard | add-report-format | [spec.md](../features/add-report-format/spec.md) | required | required | required | required |

## 切片状态

| sub-feature-id | 状态 | 后续步骤 | 阻塞原因 | 恢复条件 | 恢复后目标 |
|---|---|---|---|---|---|
| add-report-format | blocked | 恢复后 → Planner（Design + Plan） | 用户优先级调整，暂停以处理 P3 并行文件执行 | 用户要求恢复 P2 | designing |
