...
3. **separator 参数**：新增 `--override-separator <delim>`。指定时 query 头写入 `separator=<delim>` 且期望按行式回写（行内列以 `delim` 连接）；未指定时默认 value-per-line（每个值一行，即现有行为）。
...
- **新 CLI flag**：`--override-separator <delim>`；`delim` 不得含空白；仅与 `--override` 联合生效，单独使用报 UsageException。
...
- P0-5 separator：`--override-separator "|"` → query 头含 `separator=|`，期望按行式（`a|b`）。
- P0-6 默认格式不变：未指定 separator 时仍 value-per-line。
- P0-7 回归：既有 `--override` 不匹配回写、正常测试、非 override 运行不回归。
