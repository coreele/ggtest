...
| `devops` | 按需提供本地脚本和部署排障文档 | 脚本、`workflow/deploy/` |
...
归档步骤（与 `workflow/README.md`「关闭与归档」一致；features 与工作项记录一并迁入）：

1. 从 `workflow/docs/manager/STATUS.md` 活跃列表移除工作项；
2. 若存在 `workflow/docs/features/<feature-id>/`，将其移动到 `workflow/docs/archive/YYYY/<feature-id>/`；否则创建该归档目录；
3. 将 `workflow/docs/manager/<feature-id>.md` 移动为 `workflow/docs/archive/YYYY/<feature-id>/manager.md`，并修正相对链接；归档后不得在 `workflow/docs/manager/` 残留该文件；
4. 在 STATUS 归档区域记录工作项标识、最终状态和归档目录链接（目录列指向 `workflow/docs/archive/YYYY/<feature-id>/manager.md`）；
5. 仓库可用时提交归档变更。
