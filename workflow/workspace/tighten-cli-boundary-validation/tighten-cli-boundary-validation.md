...
> **本文件须保存为 `workflow/workspace/tighten-cli-boundary-validation/tighten-cli-boundary-validation.md`**，文件名与目录同名。
> 流程定义见 `workflow/WORKFLOW.md`；看板见 `workflow/STATUS.md`。
> 本工作项的全部产物平铺在 `workflow/workspace/tighten-cli-boundary-validation/`，无子目录、无版本后缀。
