...
调度主键为 `(feature-id, sub-feature-id)`。下文「切片目录」指：未拆分为 `agents/docs/features/<feature-id>/`，已拆分为 `agents/docs/features/<feature-id>/<feature-id>-<sub-feature-id>/`。已拆分时按分配的切片实施，读取该切片目录内文档。

## 输入与前置门禁

- `agents/docs/manager/<feature-id>.md`：读取工作项标识、当前切片的路径等级、Review 门禁和已持久化的 Plan 确认结果；
