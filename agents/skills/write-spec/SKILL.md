...
description: >-
  ...
  agents/docs/features/<feature-id>/spec.md (unsplit) or
  agents/docs/features/<feature-id>/<feature-id>-<sub-feature-id>/spec.md (split).
...
- 产出：
  - 未拆分：`agents/docs/features/<feature-id>/spec.md`（此时 `sub-feature-id` = `feature-id`，无需子目录）；
  - 已拆分：总览 `agents/docs/features/<feature-id>/spec.md` 与/或切片 `agents/docs/features/<feature-id>/<feature-id>-<sub-feature-id>/spec.md`；
  - 模板：`agents/docs/_templates/spec.md`。
...
必须读取用户表述、`agents/docs/manager/<feature-id>.md`、`agents/README.md`、相关源码和现有文档。执行本 Skill 不要求 `plan.md` 已存在。
...
必须遵循 `agents/docs/standards/documentation.md`。禁止编写 Plan、Design 或业务代码，禁止修改 `agents/docs/manager/STATUS.md` 或工作项记录。

## 执行与交接

1. 确认 `<feature-id>`、`<sub-feature-id>` 与 Spec 门禁。
2. 完成调研，编写对应 Spec 文件，并依据必含内容自检。
3. Git 仓库中的提交操作必须遵循 `agents/docs/standards/git.md`；非 Git 工作区跳过提交操作。
