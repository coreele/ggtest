...
| 资源 | 路径 | 加载规则 |
|---|---|---|
| 基线标准 | `agents/docs/standards/code-audit.md` | 必读 |
| 专项标准 | `agents/docs/standards/code-audit-<topic>.md` | 自动加载（见下方通配） |
| 关注项登记册 | `agents/docs/standards/code-audit-register.md` | **若存在则必读**（不当作条款标准） |
| 其他标准 | 用户点名的文件 | 仅显式指定时追加 |

**通配：** 加载 `agents/docs/standards/code-audit*.md`，但**排除** `code-audit-register.md`。  
...
| 产出 | 违规 Findings **与** 关注项（Known Issue / Tech Debt / 优化）；结论写入 `agents/docs/audit/` |
| 改动 | **禁止修改源码**（`src/` 等）。只读取证；即使发现 Critical 也不顺手修。允许写入审计报告到 `agents/docs/audit/` |

## 执行步骤

1. 枚举并完整读取 `agents/docs/standards/code-audit*.md`（排除 `code-audit-register.md`）。
...
7. 按下方模板输出报告，并写入 `agents/docs/audit/YYYY-MM-DD-<scope>.md`（scope 如 `src` 或收窄后的路径别名）。对话中可摘要，**完整结论以该文件为准**。

## 报告模板

落盘：`agents/docs/audit/YYYY-MM-DD-<scope>.md`（例如 `agents/docs/audit/2026-07-26-src.md`）。正文使用下方结构：
...
| 级别 | 标准 | 位置 | 问题 | 建议 |
|---|---|---|---|---|
| ... | agents/docs/standards/code-audit.md§N | path:line | ... | ... |
...
| 报告落盘 | 写入 `agents/docs/audit/`；这不属于「修改源码」 |
