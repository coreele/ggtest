...
描述: 审计 Finding Medium+Low §7 — `CliSession.sanitize` 真正脱敏（URL userinfo / password 字面量）；`CliOptions.toString` 对 url 做 userinfo 脱敏；补泄露证明测试。来源：`agents/docs/audit/2026-07-26-src.md`。
路径等级: standard
源分支: fix-cli-credential-redaction
目标分支: main
文档影响: agents/docs/features/fix-cli-credential-redaction/spec.md（短 Spec）；审计登记册 CA-002
...
- **approved**（2026-07-26）：用户授权；依据 `agents/docs/features/fix-cli-credential-redaction/plan.md`
