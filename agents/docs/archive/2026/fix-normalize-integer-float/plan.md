...
- 完成条件：当前为源分支；未改 `agents/docs/manager/*` / `STATUS.md`。
...
| T4 | `agents/docs/features/fix-normalize-integer-float/dev-notes.md` |
| 禁止 | Spec/Design；`agents/docs/manager/*`；`STATUS.md`；WI-2/WI-3 实现；`pom.xml` 无关改动；`sqllogictest/` 入库；本轮 commit（除非用户授权） |
