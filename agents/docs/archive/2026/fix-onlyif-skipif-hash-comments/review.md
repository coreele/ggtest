...
- 依据: `agents/docs/manager/fix-onlyif-skipif-hash-comments.md`；`plan.md`（T0–T4，A1–A5）；`dev-notes.md`；`agents/docs/standards/{documentation,quality,security,git}.md`
...
- 审阅对象:
  ...
  - `agents/docs/features/fix-onlyif-skipif-hash-comments/{plan,dev-notes}.md`
- 工作区另有 `pom.xml`、`agents/docs/manager/STATUS.md` 修改与未跟踪 `agents/docs/manager/fix-onlyif-skipif-hash-comments.md`：不纳入本项实现交付（见非阻塞备注）
- 审阅人独立复跑: 是（见「独立验证」）
...
2. `agents/docs/manager/*` / `STATUS.md` 属 Manager，不计入 A5 实现 diff。
