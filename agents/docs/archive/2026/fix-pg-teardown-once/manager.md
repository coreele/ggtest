...
描述: 审计 Tech Debt Low — PG teardown：try 内 hardFailure 时 finally 可能二次 DROP → 统一失败路径，避免双 teardown。来源：`agents/docs/audit/2026-07-26-src.md`。
路径等级: fast
源分支: fix-pg-teardown-once
目标分支: main
文档影响: agents/docs/features/fix-pg-teardown-once/；登记册 CA-006
...
- **approved**（2026-07-26）：用户授权；依据 `agents/docs/features/fix-pg-teardown-once/plan.md`
