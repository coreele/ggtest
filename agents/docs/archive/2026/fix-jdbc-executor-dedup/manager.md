...
描述: 审计 Finding Medium §4 — 抽取 `SqliteJdbcExecutor` / `PostgresJdbcExecutor` 共享逻辑（基类或 package-private helper）；保留引擎专属 ENGINE_NAME / marker。来源：`agents/docs/audit/2026-07-26-src.md`。
路径等级: standard
源分支: fix-jdbc-executor-dedup
目标分支: main
文档影响: agents/docs/features/fix-jdbc-executor-dedup/；可能更新审计登记册 CA-001
...
- **approved**（2026-07-26）：用户授权 Manager 自行决断；依据 `agents/docs/features/fix-jdbc-executor-dedup/plan.md`
