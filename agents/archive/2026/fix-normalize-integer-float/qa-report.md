...
- 不计入本项交付：工作区脏 `pom.xml`、`agents/manager/*`、未跟踪 `sqllogictest/`
...
- 残余风险:（1）合入排除无关 `pom.xml`；（2）`agents/manager/*` / 并列 WI 记录由 Manager 择机入库；（3）未跟踪 `sqllogictest/` 勿入库；（4）整文件 `failed≠0` 属 WI-2/WI-3，不阻塞本项
