...
- 依据 Spec: [agents/features/ggtest-rowwise-expected/spec.md](./spec.md)
- 依据 Design: [agents/features/ggtest-rowwise-expected/design.md](./design.md)
- 路径等级: full
...
| `README.md` | 更新 |
| `agents/features/ggtest-rowwise-expected/dev-notes.md` | Developer 更新 |

**禁止触碰**：`agents/manager/*`、`STATUS.md`、本切片 `spec.md`（合同已冻）、`.env`、`examples/demo2.slt`；不得改 MD5 拼接算法语义。
...
可选（非阻塞）：若架构总览仍列举文件级 `SeparatorRecord`，可在同 PR 增量改 `agents/features/architecture-overview/design.md`——非本 Plan 必做。
