...
| 依据 | [`plan.md`](./plan.md)、[`dev-notes.md`](./dev-notes.md)；`agents/manager/fix-jdbc-executor-dedup.md`；`agents/standards/{documentation,quality,security,git}.md` |
...
| N2 | low | CA-001 登记 | `agents/standards/code-audit-register.md` 标 `resolved` 宜在 QA Pass 后由 Manager 执行（与 Plan 一致）。 |
