...
| 依据 | [`spec.md`](./spec.md)（OQ-1 = 移除）、[`plan.md`](./plan.md)、[`dev-notes.md`](./dev-notes.md)；`agents/manager/fix-rowwise-value-per-line-compat.md`；`agents/standards/{documentation,quality,security,git}.md` |
...
行为保全核对：每值一行路径返回物理行不经 sort/flatten，与旧实现的非行式路径一致（旧代码 `rowWiseCount == 0` 时同样直接返回），与 Plan T4 一致，无回归。`ResultSorter` / `ResultHasher` / `ValueNormalizer` / CLI 未触碰，符合 Plan「不改」清单。越界检查：diff 仅含 Plan 触碰路径 + 本切片文档；未改 `agents/manager/*`、`STATUS.md`、归档。
...
| 禁止提交项 | 通过：无构建产物、无 `.env`；`examples/` 未入库；`agents/manager/*`、`STATUS.md` 未提交 |
