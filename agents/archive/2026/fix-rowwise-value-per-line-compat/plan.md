...
- 依据 Spec: [agents/features/fix-rowwise-value-per-line-compat/spec.md](./spec.md)（required，已满足；合同由用户 2026-07-26 逐条拍板）
...
- 关联工作项: [`agents/manager/fix-rowwise-value-per-line-compat.md`](../../manager/fix-rowwise-value-per-line-compat.md)
...
| T7 | Maven 套件；`agents/features/fix-rowwise-value-per-line-compat/dev-notes.md` |

**不改：** `ValueNormalizer`、`ResultHasher`、`ResultSorter`、CLI 层。  
**禁止触碰：** `agents/manager/*`、`STATUS.md`、spec.md、归档、用户本地 `examples/`。
