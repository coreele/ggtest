...
- **T5 done**：本文件。
- **禁止项遵守**：未改 CA-007 / `ResultComparer`；未改 `agents/manager/*`（Developer）；无凭据入库；未强制入库 `.temp/select*.test`。

### 变更路径

| 任务 | 路径 |
|---|---|
| T1 | `ValueNormalizer.java`；`agents/standards/code-audit-register.md` |
