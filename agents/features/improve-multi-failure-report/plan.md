...
- 依据 Spec: [agents/features/improve-multi-failure-report/spec.md](./spec.md)（Spec approved）
...
- 不做：改产品代码；改 `agents/manager/*`；启动 WI-2/WI-3。
...
3. 不做：改 `agents/manager/*` / `STATUS.md`；不改 Spec（实现无法满足则停并回 Analyst）。
...
| T3 | 无新路径 |
| T4 | `README.md`；`README.zh-CN.md`；`agents/features/improve-multi-failure-report/dev-notes.md` |
| 禁止 | WI-2/WI-3；无关 `pom.xml`；`sqllogictest/` 入库；`agents/manager/*`；`design.md` |
