...
- 依据 Spec: [docs/features/fix-cli-credential-redaction/spec.md](./spec.md)（Spec 门禁 required；用户确认 **approved**）
...
- 审计依据: `docs/audit/2026-07-26-src.md` Finding §7；登记册 **CA-002**
...
1. 新建 `docs/features/fix-cli-credential-redaction/dev-notes.md`：记录 TDD 红/绿证据、`mvn -q clean test` 摘要。
2. 将 `docs/standards/code-audit-register.md` **CA-002** 标为 `resolved`。
...
| `docs/features/fix-cli-credential-redaction/dev-notes.md` | Developer 验证回执（T5） |
| `docs/standards/code-audit-register.md` | CA-002 → `resolved`（T5） |
...
| 类别 | 更新路径或 N/A 理由 |
|---|---|
| 开发文档 | `docs/features/fix-cli-credential-redaction/dev-notes.md`（TDD 与验证回执）；`docs/standards/code-audit-register.md` CA-002 → `resolved` |
