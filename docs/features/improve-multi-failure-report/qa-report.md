...
- 不计入本项 / 勿入库：`docs/manager/*`、未跟踪 `sqllogictest/`；**`pom.xml` ≡ `main`（本项无 diff）**
...
- 残余风险:（1）字面 clean 依赖基线 toolchain（§6）；（2）`docs/manager/*` 由 Manager 择机入库；（3）`sqllogictest/` 勿入库；（4）P1-1 残差不阻塞
