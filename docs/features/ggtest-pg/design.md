...
> **前置条件**：已读 [`spec.md`](./spec.md)、工作项 [`docs/manager/ggtest-pg.md`](../../manager/ggtest-pg.md)；Spec **approved**（含 PG 与 `.env` 决议）；熟悉归档 `ggtest-core` Design 与既有 `com.ggtest.db` / `db.sqlite` / `runner` / `cli`。
