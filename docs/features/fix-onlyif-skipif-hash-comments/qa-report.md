...
| A5 | 范围仅本缺陷 | Pass | 意图变更限 parser + 对应测试 + 本目录文档；无关 `pom.xml` / `docs/manager/*` 不计入交付 |
...
- 残余风险:（1）合入须排除无关 `pom.xml`；（2）`docs/manager/*` 由 Manager 择机入库；（3）`stripTrailingHashComment` 自首个 `#` 剥离——引擎名不含 `#` 与合同一致；含 `#` 引擎名未单测（范围外）。
