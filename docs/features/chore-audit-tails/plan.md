...
- 做：在 `ValueNormalizer`（必要时 `normalizeInteger`/`normalizeReal`）Javadoc 标明非法 I/R → `"0"` / `"0.000"`，对齐 sqllogictest，非吞错缺陷。更新 `docs/standards/code-audit-register.md` CA-008：状态可仍 `accepted`；「建议下一步」改为文档已补（如 `Javadoc done; monitor only`）；更新日期。
- 不做：改归一化行为或测试期望。
...
- 完成条件：Reviewer/QA 可凭 plan + notes 复现；未改 `docs/manager/*` / `STATUS.md`。
...
| 任务 | 预期路径 |
|---|---|
| T1 | `src/main/java/com/ggtest/normalize/ValueNormalizer.java`；`docs/standards/code-audit-register.md` |
...
| T5 | `docs/features/chore-audit-tails/dev-notes.md` |
| 禁止 | ResultComparer LCS（CA-007）；`docs/manager/*`；`STATUS.md`；真实 `.env`；入库 `.temp/select*.test` |
