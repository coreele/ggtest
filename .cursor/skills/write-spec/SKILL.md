...
description: >-
  ...
  docs/features/<feature-id>/spec.md. Invoked by Analyst when Spec gate
  requires it, scheduled by Manager after work item registration and
  before /planner. Use when Spec gate requires it,
...
- 调用者：Analyst。
- 调用条件：工作项记录中的 Spec 门禁为 `required`。
- 执行时机：Manager 登记工作项后、Planner 开始 Design 或 Plan 前。
- 产出：`docs/features/<feature-id>/spec.md`，使用 `docs/_templates/spec.md`。

本 Skill 仅定义需求与规格。实施任务拆分属于 Planner 的 `plan.md`。
...
`spec.md` 必须包含背景与目标、非目标、范围与可见行为、合同、验收条件和开放问题。合同必须覆盖适用的 API、数据、状态和错误约定；不适用时标记 `N/A`。验收条件使用 Given-When-Then，并标记 P0 或 P1。
