...
| `--separator` | off | Row-wise delimiter for `--override` golden output: declares `separator=<s>` on overridden query headers and writes one row per line (columns joined by `<s>`). Requires `--override`; the value must not contain whitespace. When omitted, override writes value-per-line. Even passing queries are rewritten with the new separator when this flag is set. |
